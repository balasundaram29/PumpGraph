...
public class EntryValues implements Serializable {
    ...
    public long serialVersionUID =3L;//6644225163368733621L;//-8559476956295049665L;
    private int stdNo;
    ...
    public EntryValues(ReadingEntryPanel entryPanel) {
        ...
        for (int i = 0; i < entryTable.getRowCount(); i++) {
            for (int j = 0; j < entryTable.getColumnCount(); j++) {
               tableValueStrings[i][j]="";
                try{ 
                     
                    str= (entryTable.getValueAt(i, j)).toString();
                    tableValueStrings[i][j] =str;
               }catch(Exception ex){
